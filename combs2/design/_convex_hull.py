...
import prody as pr
import numpy as np
from ._pointTriangleDistance import point_triangle_distance as distance
...
def get_distances(pnt, hull, coords, param_dicts=None):
    S = hull.shape[0]
    if param_dicts is None:
        param_dicts = [dict() for _ in range(S)]
    return np.array([distance(pnt, coords[hull[i]], param_dicts[i]) for i in range(S)])
...
class AlphaHull:
    ...
    def calc_hull(self):
        ...
        self.param_dicts = [dict() for i in range(self.hull.shape[0])]

    def pnts_in_hull(self, pnts):
        return self._tri.find_simplex(pnts) >= 0

    def get_pnt_distance(self, pnt):
        distances = get_distances(pnt, self.hull, self.coords, self.param_dicts)
