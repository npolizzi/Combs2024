...
def find_nearest_utopian_pt(data, pareto_front, weights=None):
    ...
    if weights is None:
        #weights = np.ones(num_params).reshape(1, -1)
    ...
    for i in range(data.shape[1]):
        if len(set(data[:, i])) > 1:
            ...
            pareto_points[:, i] = 1
    # dists = cdist(pareto_points, weights)
...
# def find_nash_pt(data, pareto_front, weights=None):
#     """data should have columns that are values of individual parameters."""
#     if not isinstance(data, np.ndarray):
#         data = np.array(data)
#
...
#         pareto_points[:, i] = normalize(data[:, i])
#
#     indices_optvals = pareto_points.argmax(0)
#     optvals = pareto_points[indices_optvals]
#
#     for i, j in combinations(range(num_params), 2):
#
#
#     dists = cdist(pareto_points, weights)
#     nearest_utopian_pt = pareto_front[(dists == min(dists)).flatten()]
